...
const upload = multer()
const TALENTCOLUMNS = ["Talent_Name", "Location" ,"Seniority" ,"TechStack" ,"IOM" ,"Email" ,"LinkedInURL" ]
...
router.put('/', upload.single('file'), async function(req, res, next) {
  ...
  if(workbook.worksheets.length > 1)
    worksheet = workbook.worksheets.find(sheet => sheet.name == 'Talent Dataset')
  ...
  let queryStr = `INSERT INTO talent (${TALENTCOLUMNS.join(", ")})
VALUES`
